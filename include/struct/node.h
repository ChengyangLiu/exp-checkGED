...
class Node {
 public:
  ...
  Node(Vertex& vertex, vector<long>& neighbors, vector<long>& elabels)
      : _vertex(vertex), _neighbors(neighbors), _elabels(elabels) {}
  ...
  void getIsomorNodes(vector<Node>& g, vector<Node>& isomor_nodes) {
    ...
    for (auto& n : t1) {
      ...
      int i;
      for (i = 0; i < src_len; i++) {
        if (src_neighbors[i] != n_neighbors[i] ||
            src_elabels[i] != n_elabels[i])
          break;
      }
      if (i == src_len) {
        t2.emplace_back(n);
    ...
    for (auto& n : t2) {
      ...
      int i;
      for (i = 0; i < src_len; i++) {
        if (src_parents[i].v().id() != n_parents[i].v().id() ||
            src_p_elabels[i] != n_p_elabels[i])
          break;
      }
      if (i == src_len) {
  ...
  void getParents(vector<Node>& g, const long id, vector<Node>& parents,
                  vector<long>& elabels) {
    for (auto& n : g) {
      vector<long>& n_neighbors = n.neighbors();
      vector<long>& n_elabels = n.elabels();
